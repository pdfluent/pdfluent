...
//
// LEGACY — V1 shell. Only reachable via ?legacy URL parameter.
// Do not add features here. Scheduled for removal after feature-parity
// verification (AdminPanel, Settings).
// The active editor is src/viewer/ViewerApp.tsx + src/viewer/v3/EditorV3Shell.tsx.
// @ts-nocheck applied because this file is quarantined pending removal.
...
export function App() {
  ...
  const [textEditorEnabled, setTextEditorEnabled] = useState(
    // QA/debug default: start in text-edit mode when this key is set. The
    // toolbar toggle (toggleTextEditor) is the normal way in; this only lets
    // automated checks open straight into edit mode.
    () => {
      try {
        return localStorage.getItem("pdfluent_text_edit") === "1";
      } catch {
        return false;
      }
    },
  ...
  // Text editing draws the editable-line overlay, which only exists in the
  // single-page view. Force single-page mode whenever editing is on so the
  // overlay renders and aligns with the page (covers both the toolbar toggle
  // and the QA default above).
  useEffect(() => {
  ...
  const openPdfFromPath = useCallback(
    async (
      path: string,
      source: "dialog" | "drop_browser" | "drop_tauri" | "recent" | "open_with",
    ) => {
      const startedAt = performance.now();
  ...
  // OS "open with" / Finder double-click / file association / command-line path.
  // The Rust side emits "open-file" for warm opens and buffers cold-start opens
  // for take_pending_open. Reuse the same loader as drag-drop and the dialog.
  useEffect(() => {
    let cancelled = false;
    ...
    void (async () => {
      ...
      unlisten = await listen<string>("open-file", (e) => {
        if (e.payload) {
          appendDebugLog("info", "open_with_event", { path: e.payload });
          void openPdfFromPath(e.payload, "open_with");
        }
      });
      const pending = await invoke<string | null>("take_pending_open").catch(() => null);
      if (!cancelled && typeof pending === "string" && pending) {
        appendDebugLog("info", "open_with_pending", { path: pending });
        void openPdfFromPath(pending, "open_with");
      }
    })();
    return () => {
      cancelled = true;
    ...
    <div className="app">
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <Viewer
          ...
          currentPage={currentPage}
          scale={scale}
          viewMode={textEditorEnabled ? "single" : viewMode}
          annotationTool={annotationTool}
          highlightColor={highlightColor}
          textEditorEnabled={textEditorEnabled}
